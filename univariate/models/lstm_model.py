...
# def perform_grid_search_lstm(df, units_list, epochs_list):
#     # Preprocess the data
...
def perform_random_search_lstm(df, units_list, epochs_list, n_iter=10):
    # Preprocess the data
    min_max_scaler = MinMaxScaler(feature_range=(0, 1))
    dataset = min_max_scaler.fit_transform(df.values.reshape(-1, 1))
    ...
    for _ in range(n_iter):
        # Randomly select values for units and epochs from the lists
        units = int(np.random.choice(units_list))
        epochs = int(np.random.choice(epochs_list))
    ...
    # Evaluate the final model with the best configuration found
    final_model = build_lstm(units=best_units)
    mae, mape, mse, rmse = evaluate_final_model(final_model, train, test, epochs=best_epochs, scaler=min_max_scaler)
