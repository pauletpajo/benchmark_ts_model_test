...
# def build_rnn(time_steps=12, units=3, dense_units=1, activation=['tanh']):
#     model = Sequential()
#     model.add(SimpleRNN(units, input_shape=(time_steps, 1), activation=activation[0], return_sequences=False))
#     model.add(Dense(dense_units, activation=activation[1]))
#     model.compile(loss='mean_squared_error', optimizer='adam')
...
# def grid_search(df, units_list, epoch, time_steps=12):
...
#     mae, mape, mse, rmse = grid_search(df, units_list)
